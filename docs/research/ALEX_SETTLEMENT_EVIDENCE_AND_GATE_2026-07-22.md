...
| Payload preparation | At the handler, `execute_alex_swap` first calls `parse_gateway_x402_payload` on the request headers, then calls `build_swap_payload` and returns `{ "status": "prepared", "payload": ... }`. A valid x402-authenticated/payment request with the route's bearer authentication can reach this prepared response; bearer authentication alone is not the required x402 payload. The current payload hard-codes `SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0XBHT.alex-swap-helper-v1` and defaults `min_dy` to `1` when omitted. | **Verified current state.** This is unsigned preparation only. The hard-coded helper must not be promoted without exact network, principal, code-hash, and deployment evidence. The default must be replaced by an explicit, policy-checked nonzero minimum before any future execution. |
...
| Rehearsal script | [`scripts/alex_rehearsal.sh`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e) sends bearer authentication and a JSON body, but no required x402 payment header/payload; it also expects HTTP 501. | **Stale request and expectation.** Because the handler calls `parse_gateway_x402_payload` before building the response, this bearer-only request is rejected before it can reach `{ "status": "prepared" }`. Its expected HTTP 501 is stale: a valid x402-authenticated/payment request can reach the prepared payload. The script and handler have drifted and must be reconciled before using the rehearsal as a gate. Do not state that the current swap API always returns 501. |
...
- The current gateway handler parses x402 data before preparing a payload, while the rehearsal script sends bearer auth without that x402 payload and expects 501, so it cannot reach the prepared response. This is test/contract drift to fix, not evidence of either production execution or permanent 501 behavior.
