...
Current gateway revision: [`6838d872`](https://github.com/Conxian/conxian-gateway/tree/6838d872513b681cf88f07fc5431f02b856b6d0e).
...
| Payload preparation | `POST /api/v1/alex/swap` parses the x402 envelope, calls `build_swap_payload`, and returns `{ "status": "prepared", "payload": ... }`. The current payload hard-codes `SP3K8BC0PPEVCV7NZ6QSRWPQ2JE9E5B6N3PA0XBHT.alex-swap-helper-v1` and defaults `min_dy` to `1` when omitted. | **Verified current state.** This is unsigned preparation only. The hard-coded helper must not be promoted without exact network, principal, code-hash, and deployment evidence. The default must be replaced by an explicit, policy-checked nonzero minimum before any future execution. |
...
| Simulation | `SimulatedAlexClient` returns quote `100` and the literal `txid_alex_simulated_swap_rehearsal`. | **Verified test behavior, not production evidence.** A simulated quote or fake txid cannot satisfy activation. |
| Rehearsal script | [`scripts/alex_rehearsal.sh`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/scripts/alex_rehearsal.sh) says the swap should return HTTP 501 until signer integration exists. | **Stale expectation.** The current handler returns a prepared payload rather than calling `execute_swap`; therefore a successful authenticated request is expected to be a preparation response, not the script's expected 501. The script and handler have drifted and must be reconciled before using the rehearsal as a gate. Do not state that the current swap API always returns 501. |
...
| Gateway ALEX engine | Quote interface, payload construction, simulated client, and fail-closed execution boundary. | [`internal/engine/src/stacks/alex.rs`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/internal/engine/src/stacks/alex.rs) |
| Gateway API/routes/config | `/api/v1/alex/quote`, `/api/v1/alex/swap`, middleware protection, and `ALEX_API_URL`/network defaults. | [`handlers.rs`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/internal/api/src/handlers.rs), [`routes.rs`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/internal/api/src/routes.rs), [`config.rs`](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/cmd/gateway/src/config.rs) |
...
- The issue's `CON-1474` link is unresolved/not found in the checked Linear workspace. The active, resolved gate is [`CON-1529`](https://linear.app/conxian-labs/issue/CON-1529/p1-research-and-implement-verified-alex-production-activation), which links to `Conxian#526`.
- The gateway package [`@conxian/client-sdk` 0.1.4](https://github.com/Conxian/conxian-gateway/blob/6838d872513b681cf88f07fc5431f02b856b6d0e/packages/client-sdk/package.json) has no ALEX-specific quote, settlement-intent, status, or reconciliation API.
...
| [Stacks Foundation June 2025 incident statement](https://stacks.org/june-6-alex-incident) | Confirms the June 2025 incident was at the ALEX application layer, not a Stacks protocol vulnerability. | It is not a complete ALEX post-mortem or current pool inventory. |
...
- Add typed quote, risk decision, and unsigned settlement-intent records.
- Require exact principal allowlists, quote expiry, nonzero `min-dy`, deny-mode postconditions, exposure caps, and a deterministic policy decision.
- Add negative tests for wrong network, wrong principal, zero `min-dy`, expired quote, stale code hash, paused/blocklisted state, excessive price impact, and exposure-cap breach.
- Produce reproducible read-only evidence bundles; no mainnet execution acceptance yet.
...
- The current gateway handler and rehearsal script disagree: the handler prepares a payload, while the script expects 501. This is a test/contract drift to fix, not evidence of either production execution or permanent 501 behavior.
