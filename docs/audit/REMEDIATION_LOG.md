...
## 13. Workspace-Wide Tracked Artifact & Sensitive Config Verification Hardening (2026-09-13)
- **Remediation**: Hardened `scripts/verify_tracked_artifacts.py` to recursively inspect all tracked repository files against prohibited directory components (`node_modules`, `test-results`, `playwright-report`, `dist`, `build`, `target`, `.next`, `coverage`, `.pytest_cache`, `htmlcov`), sensitive environment configuration naming conventions (`.env*` except `.env.example`), and generated/key file extensions (`*.key`, `*.pem`, `*.pfx`, `*.p12`, `*.secret`, `*.keystore`, `*.db`, `*.sqlite`, `*.pyc`, `*.pyo`, `*.pyd`, `*.lcov`, `*.bak`, `*.tmp`).
- **Risk Addressed**: Root-scoped pathspecs (such as `git ls-files node_modules/`) previously created blind spots for nested workspace packages (e.g. `apps/control-plane/node_modules/` or `apps/control-plane/.env.local`), allowing prohibited artifacts and sensitive configuration in monorepo subdirectories to bypass CI verification.
- **Unit Test Coverage**: Added `test_detects_nested_workspace_tracked_artifacts` and `test_detects_nested_workspace_tracked_env_files` to `tests/test_verify_tracked_artifacts.py`, verifying robust detection across all monorepo depth levels.
- **Verification**: Executed `python3 -m unittest tests/test_verify_tracked_artifacts.py` (7/7 tests passed), `python3 scripts/verify_tracked_artifacts.py` (0 prohibited artifacts), `python3 scripts/verify_contamination_guard.py` (95 files clean), and `python3 scripts/verify_release_hygiene.py` (version v0.1.5 verified).
