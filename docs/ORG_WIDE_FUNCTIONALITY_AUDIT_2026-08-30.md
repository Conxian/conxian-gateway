...
- [x] **G-6** — KB freshness: audit doc, `PORTFOLIO_MAP.md` slug corrections,
  `CROSS_REPO_STATUS.md` 2026-08-30 session update.
- [x] **Client SDK** — `generatePacs008Payment` endpoint corrected to the wired
  `/iso20022/payment` route.
- [x] **SYI** — read-only `/api/v1/treasury/syi` handler + route + schema
  alignment (no fabricated quotes).
- [x] **Gateway #189 (wiring)** — BitVM3 fail-closed registry wiring (PR #351).

### Blocked — external / human decision gates (not agent-completable)
| Item | Blocker |
|------|---------|
| enclave-sdk #202, #240, #241, #242 | Independent security review & release-acceptance evidence |
| Conxian #488, #496, #527, #529, #530, #532 | Partnership fee/legal/commercialization decisions |
| conxian-business #934–#938 | BOS handoff gates (business acceptance, owner sign-off) |
| conxius-platform #1167, #1168 | Strategic direction (Core Directive #7 protocol handoff) |
| .github #43, #47, #53, #60 | Org operating model & ruleset governance |
| conxian-nexus #174 | License policy decision |

### Actionable cross-repo (owner + next step)
| Item | Owner | Next step |
|------|-------|-----------|
| G-1 enclave-sdk v2.0.17 re-pin | `lib-conxian-core` | Bump SDK tag in `lib-conxian-core` Cargo.toml, release, then Gateway re-pins |
| G-2 BN254 ↔ BLS12-381 verifier contract | gateway + nexus | Agree curve/VK/public-input/state-root + verifier-ownership contract |
| conxian-nexus #251 IdempotencyStore | nexus | Set `DATABASE_URL`, run migration, add live-DB conformance suite |
| conxius-wallet #444 value-op gate | wallet | Add centralized value-operation policy gate; quarantine software signer |
| conxius-enclave-sdk #271 LDK execution | enclave-sdk | Implement LDK payment execution (structural, 307 lines) |
| conxian.github.io #3 Pages restore | site | Restore Pages deploy under immutable action pinning |

---

## 8. Per-Issue Best-Option Triage (all 42 open issues)

Legend — **Code**: agent-implementable via PR · **Blocked**: human/external decision gate.

| # | Issue | Best option | Status |
|---|-------|-------------|--------|
| 1 | conxian.github.io #3 | Pin Pages actions to immutable SHAs | ✅ PR [#6](https://github.com/Conxian/conxian.github.io/pull/6) |
| 2 | conxian-gateway #189 | BitVM3 fail-closed research lane | ✅ PR [#351](https://github.com/Conxian/conxian-gateway/pull/351) |
| 3 | conxius-platform #1082 | Port the 8 missing `scripts/verify_*.py` from gateway | ✅ Assessed stale (see note) |
| 4 | Conxian #515 | Consolidate to one CODEOWNERS + reconcile team slugs | Code (needs admin for slug verify) |
| 5 | conxius-wallet #444 | Add centralized value-op policy gate | Code (P0) |
| 6 | conxius-enclave-sdk #271 | Implement LDK payment execution (307 lines) | Code |
| 7 | conxian-nexus #251 | Wire IdempotencyStore + live-DB conformance | ✅ PR [#258](https://github.com/Conxian/conxian-nexus/pull/258) + Neon wired |
| 8 | Conxian #507 | sBTC Vault Clarity implementation | Code (large) |
| 9 | Conxian #488/#496/#529/#530 | Partnership fee contracts + ledger + SDK | Code (blocked by #527 policy) |
| 10 | conxius-wallet #356/#357 | CI/CD baseline + tech-debt reduction | Code |
| 11 | conxian_ui #161 | Authoritative deploy evidence doc | Code/docs |
| 12 | conxian_market #8 | Treasury transparency dashboard | Code (feature) |
| 13 | conxius-platform #958 | Org-wide auto-merge enablement | Blocked (admin) |
| 14 | conxius-enclave-sdk #202/#240/#241/#242 | Independent security review & release acceptance | Blocked (review) |
| 15 | conxius-enclave-sdk #200 | Harden WASM secret boundary + runtime evidence | Blocked (runtime) |
| 16 | Conxian #527/#532 | Partnership fee/legal/commercialization decision | Blocked (legal) |
| 17 | conxius-platform #1167/#1168 | Protocol handoff & founder-rights routing | Blocked (strategy) |
| 18 | conxius-platform #854/#1212/#1223 | Rulesets / push protection / stale branches | Blocked (admin/governance) |
| 19 | .github #43/#47/#53/#60 | Operating model, metadata, notices, license | Blocked (governance) |
| 20 | conxian-nexus #174 | License policy restoration | Blocked (legal) |
| 21 | conxian-business #934–#940, #989 | BOS gates, FIBO provenance, position research | Blocked (business) |

### Session plan (best option per session)

1. **This session** — connectivity + research + `conxian.github.io` #3 (done: PR #6).
2. **#1082 assessed** — workflow already refactored; no `hashFiles()` guards remain; the 2 scripts of the 8 that survived are present and invoked by `bos-production-guard.yml`. Comment posted recommending closure (or explicit re-port of the 6 removed checks if still required).
3. **Next** — `conxius-wallet` #444 (value-op gate), `conxius-enclave-sdk` #271 (LDK), `Conxian` #507 (sBTC vault), `conxian_ui` #161 (deploy evidence), `conxian_market` #8 (dashboard).

### Issue-trail status (2026-08-30)

Every open issue now carries an on-issue status trail (comment or linked PR):

- **PR + comment** — `conxian.github.io` #3 (PR #6), `conxian-gateway` #189 (PR #351), `conxian-nexus` #251 (PR #258).
- **Assessed stale/resolved + comment** — `conxius-platform` #1082, `Conxian` #515.
- **Assessed + comment (specific next step)** — 12 codeable/gated items (`wallet` #356/#357/#444, `enclave-sdk` #271, `Conxian` #488/#496/#500/#507/#529/#530, `conxian_ui` #161, `conxian_market` #8).
- **Blocked + comment (owner + blocker)** — 25 human/admin/legal/security/strategy-gated items.

Total: 42/42 issues carry a current trail.
