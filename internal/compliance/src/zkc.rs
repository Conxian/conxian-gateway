...
impl ZkcVerifier {
    ...
    pub fn verify_zkml(&self, proof: &ZkmlProof) -> ConxianResult<bool> {
        ...
        let image_id_bytes = Vec::from_hex(image_id_hex)
            .map_err(|_| ConxianError::Compliance("Invalid proof image format".into()))?;
        let image_id = Self::bytes_to_digest_words_le(&image_id_bytes)?;
    ...
    fn bytes_to_digest_words_le(bytes: &[u8]) -> ConxianResult<[u32; 8]> {
        const DIGEST_LEN: usize = 32;
        if bytes.len() != DIGEST_LEN {
            return Err(ConxianError::Compliance(
                "Internal verification length mismatch".into(),
            ));
        }

        let mut words = [0u32; 8];
        for (idx, chunk) in bytes.chunks_exact(4).enumerate() {
            words[idx] = u32::from_le_bytes(chunk.try_into().map_err(|_| {
